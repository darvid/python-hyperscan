...
static int match_handler(unsigned int id, unsigned long long from,
  ...
  PyObject *rv = PyObject_CallFunction(cctx->callback, "IIIIO", id, from, to,
                                       flags, cctx->ctx);
  int halt = 1;
  if (rv == NULL) {
    cctx->success = 0;
...
static void Database_dealloc(Database *self) {
  ...
  if (scratch != NULL) hs_free_scratch(scratch);
...
static int Database_init(Database *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|Oi", kwlist, &self->scratch,
                                   &self->mode))
    return -1;
  return 0;
}
...
static PyObject *Database_compile(Database *self, PyObject *args,
                                  PyObject *kwds) {
  PyObject *oexpressions = Py_None, *oflags = Py_None, *oflag = Py_None,
           *oids = Py_None, *oliteral = Py_False;
  int elements = -1;

  static char *kwlist[] = {"expressions", "ids",     "elements",
                           "flags",       "literal", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OIOO", kwlist, &oexpressions,
                                   &oids, &elements, &oflags, &oliteral))
    return NULL;
  ...
  Py_BEGIN_ALLOW_THREADS if (PyObject_IsTrue(oliteral)) {
  ...
  if (self->scratch == Py_None)
    self->scratch = PyObject_CallFunction((PyObject *)&ScratchType, "O",
                                          (PyObject *)self, 0);
  else {
...
static PyObject *Database_scan(Database *self, PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = {
      "data", "match_event_handler", "flags", "context", "scratch", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s#|OIOO", kwlist, &data,
                                   &length, &ocallback, &flags, &octx,
                                   &oscratch))
    return NULL;
  py_scan_callback_ctx cctx = {ocallback, octx, 1};
  Py_BEGIN_ALLOW_THREADS err =
      hs_scan(self->db, data, length, flags,
  ...
  Py_END_ALLOW_THREADS if (!cctx.success) { return NULL; }
  HANDLE_HYPERSCAN_ERR(err, NULL);
  Py_RETURN_NONE;
}
...
static PyObject *Database_stream(Database *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|IOO", kwlist, &flags,
                                   &ocallback, &octx))
    return NULL;
  PyObject *stream =
      PyObject_CallFunction((PyObject *)&StreamType, "OIOO", (PyObject *)self,
...
static void Stream_dealloc(Stream *self) {
  if (self->cctx != NULL) free(self->cctx);
  Py_TYPE(self)->tp_free((PyObject *)self);
}
...
static PyObject *Stream_close(Stream *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|O!OO", kwlist, &oscratch,
                                   &ScratchType, &ocallback, &octx))
    return NULL;
  if (PyObject_Not(oscratch)) oscratch = ((Database *)self->database)->scratch;
...
static PyObject *Stream_scan(Stream *self, PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = {"data",    "flags", "scratch", "match_event_handler",
                           "context", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s#|IOOO", kwlist, &data,
                                   &length, &flags, &oscratch, &ocallback,
                                   &octx))
    return NULL;

  if (PyObject_Not(ocallback)) ocallback = self->cctx->callback;
  if (PyObject_Not(octx)) octx = self->cctx->ctx;
  ...
  if (PyObject_Not(oscratch))
    ...
    if (!PyObject_IsInstance(oscratch, (PyObject *)&ScratchType)) {
      PyErr_SetString(PyExc_TypeError,
                      "scratch must be a hyperscan.Scratch instance");
      return NULL;
    }
    scratch = ((Scratch *)oscratch)->scratch;
  }
  ...
  err = hs_scan_stream(self->identifier, data, length, flags, scratch,
                       ocallback == Py_None ? NULL : match_handler,
                       ocallback == Py_None ? NULL : (void *)&cctx);
  Py_END_ALLOW_THREADS;
  HANDLE_HYPERSCAN_ERR(err, NULL);
...
static void Scratch_dealloc(Scratch *self) {
  if (self->scratch != NULL) hs_free_scratch(self->scratch);
  Py_TYPE(self)->tp_free((PyObject *)self);
}

static PyObject *Scratch_set_database(Scratch *self, PyObject *args,
                                      PyObject *kwds) {
...
static PyMethodDef Scratch_methods[] = {
    ...
    {"set_database", (PyCFunction)Scratch_set_database,
...
static PyMethodDef Hyperscan_methods[] = {
    ...
    {"loadb", (PyCFunction)loadb, METH_VARARGS | METH_KEYWORDS,
     "loadb(buf)\n"
...
MOD_INIT(_hyperscan) {
  ...
  MOD_DEF(m, "_hyperscan", "Hyperscan bindings for CPython.",
          Hyperscan_methods);
  ...
  if (PyType_Ready(&DatabaseType) < 0 || PyType_Ready(&ScratchType) < 0 ||
